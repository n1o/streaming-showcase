...
users = ["user_%s" %i for i in range(100)]
...
async def generate_event_1(users, producer):
    ...
    def gen_event(user):
        return {
            "userId": user,
            "userValue1": round(n11.rvs() if val1.rvs() == 1 else n12.rvs(), 2),
    ...
    while True:
        ...
        for user in random.sample(users, size):
            user_event = gen_event(user)
            producer.produce(topic = "events_1",  key = {"user": user}, value=user_event)
...
async def generate_event_2(users, producer):
    ...
    def gen_event(user):
        return {
            "userId": user,
            "userValue3": round(n11.rvs() if val1.rvs() == 1 else tt.rvs(), 2),
    ...
    while True:
        ...
        for user in random.sample(users, size):
            user_event = gen_event(user)
            producer.produce(topic = "events_2",  key = {"user": user}, value=user_event)
